...
class Usuario extends Model
{
    ...
    public function set_enlace($data = array())
    {
        ...
        $this->query = "INSERT INTO users(user, password)
    ...
    public function register($user_data = array()){
        ...
        $this->query = "INSERT INTO users(user, password, nombre, email)
        VALUES(:user, :password)";
        //$this->parametros['id']= $id;
        $this->parametros['user'] = $user;
        $this->parametros['password'] = $password;
    ...
    public function delete($user_data = array())
    {
        ...
        $this->query = "DELETE FROM users WHERE user = :user and password = :password";
        $this->parametros['user'] = $user;
        $this->parametros['password'] = $password;
        $this->get_results_from_query();
    ...
    public function login($user_data = array())
    {
        ...
        $this->query = "SELECT * FROM users WHERE user = :user and psw = :password";
        $this->parametros['user'] = $user;
        $this->parametros['password'] = $password;
        $this->get_results_from_query();
        if (count($this->rows) == 1) {
    ...
    public function bloquearUsuario($user = ''){
        $this->query = "UPDATE users SET bloqueado = 1 WHERE user = :user";
        $this->parametros['user'] = $user;
        $this->get_results_from_query();
    ...
    public function desbloquearUsuario($user = ''){
        $this->query = "UPDATE users SET bloqueado = 0 WHERE user = :user";
        $this->parametros['user'] = $user;
        $this->get_results_from_query();
    ...
    public function comprobarBloqueado($user = ''){
        $this->query = "SELECT bloqueado FROM users WHERE user = :user";
        $this->parametros['user'] = $user;
        $this->get_results_from_query();
    ...
    public function getusersBloqueados(){
        $this->query = "SELECT * FROM users WHERE bloqueado = 1";
        $this->get_results_from_query();
        return $this->rows;
